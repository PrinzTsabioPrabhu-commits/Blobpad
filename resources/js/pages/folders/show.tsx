...
export default function FolderShow({ folder, notes: notesPaginated }: FolderShowProps) {
    return (
        <>
            ...
            <div className="flex h-full flex-1 flex-col gap-6 overflow-x-auto bg-gradient-to-b from-amber-50 to-yellow-50 p-6 md:p-10">
                ...
                <div className="flex items-center gap-4">
                    <Link href={folders()} className="flex h-12 w-12 items-center justify-center rounded-xl border-4 border-gray-900 bg-white shadow-[4px_4px_0px_rgba(0,0,0,1)] transition-all hover:translate-y-1 hover:shadow-[0px_0px_0px_rgba(0,0,0,1)]">
                        <ArrowLeft size={22} strokeWidth={3} />
                    </Link>
                    <div>
                        <h1 className="text-3xl font-black tracking-tight text-gray-900">📂 {folder.name}</h1>
                        <p className="mt-1 text-sm font-bold text-gray-600">{notesPaginated.data.length} catatan dalam folder ini</p>
                    </div>
                </div>
                ...
                {notesPaginated.data.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-20 text-gray-500">
                        ...
                        <p className="mt-2 font-bold">Belum ada catatan di folder ini</p>
                    </div>
                ) : (
